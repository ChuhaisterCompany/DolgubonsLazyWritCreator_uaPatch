...
function WritCreater.writCompleteStrings() -- Vital for translation
	local strings = {
	["place"] = "Розмістіть товари",
	["sign"] = "Підписати маніфест",
	["masterPlace"] = "Я закінчив ",
	["masterSign"] = "<Закінчити роботу.>",
	["masterStart"] = "<Прийняти контракт.>",
...
function WritCreater.langTutorial(i) 
	local t = {
		[5]="Також є кілька речей, які вам слід знати.\nПо-перше, /dailyreset це команда косої риски, яка скаже вам\nскільки часу залишилося до наступного щоденного скидання сервера.",
		[4]="Нарешті, ви також можете вибрати деактивацію або\nактивувати цю аддон для кожної професії.\nЗа замовчуванням увімкнено всі відповідні ремесла.\nЯкщо ви хочете дещо вимкнути, перевірте налаштування.",
		[3]="Далі вам потрібно вибрати, чи хочете ви це бачити\nвікно під час використання станції ремесла.\nУ вікні буде вказано, скільки матеріалів знадобиться для замовлення, а також скільки у вас зараз є.",
		[2]="Перше налаштування, яке потрібно вибрати, це якщо ви\nбажаєте використовувати AutoCraft.\nЯкщо увімкнено, коли ви заходите на станцію ремесла, додаток розпочне створення.",
		[1]="Ласкаво просимо до Dolgubon's Lazy Writ Crafter!\nЄ кілька налаштувань, які слід вибрати спочатку.\n Ви можете змінити налаштування будь-коли\n час у меню налаштувань.",
	}
	return t[i]
end
...
function WritCreater.langTutorialButton(i,onOrOff) -- sentimental and short please. These must fit on a small button
	local tOn = 
	{
		[1]="Використовувати, за замовчуванням",
		[2]="Увімкнути",
		[3]="Показати",
		[4]="Продовжити",
		[5]="Закінчити",
...
if l then
	...
	WritCreater.strings.smithingReqM = function (amount, _,more)
		...
		return zo_strformat( "Ремесло використовуватиме <<1>> <<4>> (|cf60000Вам потрібно <<3>>|r)" ,amount, type, more, DivineMat) end
	WritCreater.strings.smithingReqM2 = function (amount, _,more)
		local craft = GetCraftingInteractionType()
		DivineMat = DivineMats[craft]
		return zo_strformat( "Так само, як <<1>> <<4>> (|cf60000Вам потрібно <<3>>|r)" ,amount, type, more, DivineMat) end
	WritCreater.strings.smithingReq = function (amount, _,more) 
		local craft = GetCraftingInteractionType()
		DivineMat = DivineMats[craft]
		return zo_strformat( "Ремесло використовуватиме <<1>> <<4>> (|c2dff00<<3>> доступний|r)" ,amount, type, more, DivineMat) end
	WritCreater.strings.smithingReq2 = function (amount, _,more) 
		local craft = GetCraftingInteractionType()
		DivineMat = DivineMats[craft]
		return zo_strformat( "Так само, як <<1>> <<4>> (|c2dff00<<3>> доступний|r)" ,amount, type, more, DivineMat) end
end
